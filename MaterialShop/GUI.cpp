...
void GUI::init() {
    ...
    // 关键修改：Pay Button Y 坐标固定在底部日志区上方（已向上移动）
    int fixed_payButton_y = getheight() - 310 - 60 - 100; // 约 getheight() - 470
...
void GUI::initPaymentButtons() {
    ...
    // 3. 重新定位 Pay Button (这里仅用于确保 payButton.y 仍然被使用，但实际上它在 init() 中已经被固定了)
    // 这一行代码理论上不需要修改 payButton.y，但为了逻辑完整性，保留此计算方式，
    // 确保 future proofing，并使用新的间隔值。
    payButton.y = startY + paymentRows * (btnH + gap) + verticalGap; // 使用新的 60px 间隔
}
...
void GUI::render() {
    ...
    solidrectangle(infoX, 50, infoX + infoW, getheight() - 320);
    ...
    for (auto p : payments) {
        ...
        currentY += 22;
        if (currentY > getheight() - 340) break; // 防止越界覆盖日志区
    }
    ...
    // 6.5 绘制购物车面板（在左侧科目下方，靠近底部日志区之上）
    {
        ...
        int cartW = 400;
        int cartH = 170;
        int cartY = getheight() - 300 - cartH - 10; // 在日志区之上留 10px 间隔
    ...
    fillrectangle(0, h - 300, w, h);

    settextcolor(GREEN);
    settextstyle(18, 0, "Consolas");
    RECT r = { 20, h - 290, w - 20, h - 10 };
